...
- **Reading Google's actual route polyline.** Google does not expose the route geometry on the page. Two options, in increasing fidelity:
  ...
  3. *(Rejected)* **Intercepting Google's internal XHR/fetch.** Works today but is undocumented and breaks every time Google ships a Maps update. We will not depend on it.
...
- No injecting markers *into* Google's WebGL canvas (only on top, as our own DOM).
